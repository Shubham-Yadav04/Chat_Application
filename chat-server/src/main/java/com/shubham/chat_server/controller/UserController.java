...
public class UserController {
    ...
    public User getAuthenticatedUser(@CookieValue("access_token") String token){
        String username= jwtService.extractToken(token).getSubject();
    ...
    public ResponseEntity<?> createUser(@RequestBody User user){
        ...
        String accessToken=jwtService.generateToken(user.getUsername(),1000*60*60); // Access token
        String refreshToken=jwtService.generateToken(user.getUsername(),1000*60*60*24*10); // Refresh token for 10 days
    ...
    public ResponseEntity<?> getRandomUser(@CookieValue("access_token") String accessToken){
        String username= jwtService.extractToken(accessToken).getSubject();
        List<User> result=  userService.getRandomUser( username);
        if(result!=null){
